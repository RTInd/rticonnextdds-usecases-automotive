    ...
    DDS_DataReader * reader)
{
    ...
    for (i = 0; i < dds_perception_ObjectsDetectedSeq_get_length(&sample_seq); ++i)
    {
        ...
        if (sample_info->valid_data)
        {
            ...
            printf("Valid ObjectsDetected sample received\n");
            *total_samples += 1;
        ...
        {
            printf("\nObjectsDetected sample received\n\tINVALID DATA\n");
        }
    }
    ...
    DDS_DataReader * reader)
{
    ...
    for (i = 0; i < dds_sensing_gnss_BasicSeq_get_length(&sample_seq); ++i)
    {
        ...
        if (sample_info->valid_data)
        {
            ...
            printf("Valid GNSS Basic sample received\n");
            *total_samples += 1;
    ...
    DDS_DataReader * reader)
{
    ...
    for (i = 0; i < dds_perception_TrajectorySeq_get_length(&sample_seq); ++i)
    {
        ...
        if (sample_info->valid_data)
        {
            ...
            printf("Valid Trajectory sample received\n");
            *total_samples += 1;
    ...
    DDS_Long count)
{
    ...
    {
        printf("failed to assert remote subscription(perception_dp::Pose3D)\n");
        goto done;
    }
    ...
    if (DDS_RETCODE_OK != DPSE_RemoteSubscription_assert(
        ...
        dds_physics_Pose3D_real_get_key_kind(dds_physics_Pose3D_realTypePlugin_get(), NULL)))
    {
        printf("failed to assert remote subscription(sceneEval_dp::Pose3D)\n");
        goto done;
    }
    ...
    */
    ...
    if (retcode != DDS_RETCODE_OK)
    {
        printf("failed to assert remote publication(stimAndPrint_sp::Basic)\n");
        goto done;
    }
    ...
    if (retcode != DDS_RETCODE_OK)
    {
        printf("failed to assert remote publication(egoMotion_dp::Trajectory)\n");
        goto done;
    }
    ...
    for (i = 0; (application->count <= 0) || (i < application->count); ++i)
    {
        ...
        {
            printf("Written Pose3D sample %d\n",(i+1));
        }
